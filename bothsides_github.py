...
def wait_for_market_open():
    ...
    wait_secs = int((market_open - now).total_seconds())
    print("Market opens at: " + str(market_open.strftime("%d %b %Y %H:%M:%S")))
    print("Waiting " + str(wait_secs) + "s...")

    while datetime.now() < market_open:
        remaining = int((market_open - datetime.now()).total_seconds())
        if remaining % 30 == 0:
            print(str(remaining) + "s remaining...")
        time.sleep(1)
